...
class AdminUserController {
    ...
    search(req, res, next) {
        try {
            ...
            Promise.all([
                    User.find({ fullname: { $regex: new RegExp((req.query.q ? req.query.q : ''), "i") } }).sort({ createdAt: -1 }),
                    User.countDocuments({ fullname: { $regex: new RegExp((req.query.q ? req.query.q : ''), "i") } }).sort({ createdAt: -1 }),
